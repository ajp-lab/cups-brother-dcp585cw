...
- A **static IP address** assigned to your Brother DCP-585CW printer in your local router setup (e.g., default is set to `192.168.50.10`).

The official Brother .deb driver packages needed are already downloaded and placed inside the `./drivers/` folder:
...
Ensure the IP address in the script `01-setup-dcp585cw.sh` matches your printer's static IP (default: `192.168.50.10`) - if not - change it directly in this script. Simple as that. Then, execute it:
...
Once configured and tested, add the network printer in macOS System Settings to start printing and scanning over your local network.

## Web Interface
After setup and having the container running you can access the CUPS Web Administration panel at:
